import { f as fixImageUrl } from "./fixImageUrl.js";
import { d as defaultChannel, s as saleorApiUrl } from "./auth.js";
function parseEditorJs(editorJson) {
...
async function fetchProducts(limit = 4) {
  ...
  return nodes.map((item) => ({
    ...
    discount: null,
    // Saleor doesn't handle discounts natively here, set manually or via metadata
    rating: 4.5
    // You can mock ratings or store them in metadata
  }));
}
async function fetchProductById(id, channel = defaultChannel) {
  ...
  const getUniqueAttributeValues = (keys) => {
    return [
      ...new Set(
        allAttributes.filter((a) => matchAttribute(a, keys)).flatMap((a) => a.values.map((v) => v.name))
      )
    ];
  };
...
async function fetchProductsByCategorySlug(slug, limit = 4) {
  const res = await fetch(saleorApiUrl, {
    method: "POST",
...
async function fetchRelatedProducts(categoryId, limit = 20, excludeId, channel = defaultChannel) {
  const res = await fetch(saleorApiUrl, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
...
async function searchProducts(queryText, limit = 50) {
  const query = `
...
export {
  fetchProducts as a,
  fetchProductById as b,
  fetchRelatedProducts as c,
  fetchProductsByCategorySlug as f,
  searchProducts as s
};
