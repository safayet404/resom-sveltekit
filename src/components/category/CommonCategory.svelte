<script>
  ...
  async function onViewAll() {
    ...
    try {
      ...
      if (updated.products.length < currentLimit) {
        hasMore = false;
      }
    } catch (err) {
...
<section class="mx-auto container mt-10 px-4">
  ...
  >
    {#each product as item}
      ...
      >
        <div class="transition-transform duration-300 hover:scale-105 relative">
          <img
            alt="product-img"
        ...
        <div class="flex font-poppins gap-4">
          {#if item?.discount}
            <p class="font-semibold text-sm sm:text-base md:text-xl mt-2">
              <!-- ${(
                                item.price -
