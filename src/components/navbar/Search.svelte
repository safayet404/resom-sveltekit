...
<section class="hidden md:flex">
  <div>
    <form on:submit={handleSearch} class="flex items-center justify-end h-10 mt-1">
      <button
        type="button"
        on:click={focusInput}
        class="text-3xl cursor-pointer bg-transparent border-none p-0 m-0"
      ...
      <!-- <input bind:value={searchQuery} bind:this={searchInput} /> -->

      <input
        bind:value={searchQuery}
        placeholder="Type your search"
        class="outline-none bg-transparent flex-grow pl-4"
    ...
    {#if showDropdown && searchResults.length > 0}
      <div
        class="absolute z-50 bg-white shadow rounded mt-2 w-auto border max-h-[500px] overflow-auto"
      >
        ...
        {#each searchResults as item (item.id)}
          <div
            ...
            class="flex items-center justify-between lg:gap-12 xl:gap-36 px-3 py-2 hover:bg-gray-100 cursor-pointer"
