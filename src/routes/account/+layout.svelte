...
<script>
  ...
  const menuItems = [
    ...
    { label: "My Orders", icon: orderIcon, path: "orders" },
    { label: "Security", icon: securityIcon, path: "security" },
    { label: "Payment", icon: paymentIcon, path: "payment" },
    // { label: "Affiliate", icon: affiliateIcon, path: "affiliate" },
    { label: "Need Help", icon: helpIcon, path: "help" },
  ];
  ...
  <div class="flex mt-20 bg-[#EFEFEF] rounded-lg min-h-screen">
    ...
    <aside class="w-64 border-r p-4 space-y-4">
      {#each menuItems as item}
        <a
          class="flex gap-3 p-2 rounded-2xl hover:bg-gray-100 font-medium"
      ...
      {/each}
      <hr />
      <a class="text-[#808080] flex gap-3 font-medium p-2" href="/logout"
        >{@html logoutIcon} Log Out</a
      >
    </aside>
